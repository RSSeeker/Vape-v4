...
| File | Description |
| --- | --- |
| `Vape-v4.21.31.exe` | GUI single-file loader (embeds the complete DLL, Java payload and icon); filename varies with the version (e.g. `Vape-v4.21.31.exe`) |
...
- Command-line injector: `Vape-v4.21.31.exe -nogui [pid]` — without a pid it opens the process picker; with a pid it injects directly
...
The complete test bundle outputs to `build/injection/` (the filename varies with the project version, e.g. `Vape-v4.21.31.exe`):

```text
Vape-v4.21.31.exe   GUI single-file loader (embeds the DLL and all resources)
README.md
```
