...
| 文件 | 说明 |
| --- | --- |
| `Vape-v4.21.29.exe` | GUI 单文件加载器（内嵌完整 DLL 与 Java 载荷 + 图标），文件名随版本号变化（如 `Vape-v4.21.29.exe`） |
...
- 命令行注入器：`Vape-v4.21.29.exe -nogui [pid]` —— 不带 pid 弹出进程选择器，带 pid 直接注入
...
以下问题经大量排查，根因主要是 **1.20.1 的渲染/映射与代码目标版本（1.21.x / 26.x）差异较大**，
涉及多个核心映射错位（实体坐标、包围盒、相机旋转、PoseStack、投影 getter 等），**修复难度高、暂未解决**：
...
完整测试包输出到 `build/injection/`（文件名随项目版本号变化，如 `Vape-v4.21.29.exe`）：

```text
Vape-v4.21.29.exe   GUI 单文件加载器（内嵌 DLL 与全部资源）
README.md
```
